...
function validateuserId(request, response, next){
  ...
  if (!isUuid(id)) 
     return (response.status(400).json({ error: 'Identificardor(ID) invalido. (Middleware)' }));
...
app.get('/users', (request, response) => {
  ...
  results = name ?
    users.filter(user => user.title.includes(name)) :
    users;

  // Filtro (Query inserida no insomnia) por owner
  results = email ?
    results.filter(user => user.owner.includes(email)) :
    results;
